...
from acsl.module.policy_dist import CategoricalActorDistribution
...
class TrainConfig:
    job_name: str = "2048"
    name: str = "bc10-stochastic"
    # Experiment
    device: str = "cuda:0"
    env: str = "2048-v0"  # OpenAI gym environment name
    seed: int = 0  # Sets Gym, PyTorch and Numpy seeds
    eval_freq: int = int(4e3)  # How often (time steps) we evaluate
    log_freq: int = int(1e3)
    n_episodes: int = 10  # How many episodes run during evaluation
    max_timesteps: int = 150000  # Max time steps to run environment
...
class Actor(nn.Module):
    def __init__(self, state_dim: int, action_dim: int):
        ...
        self.head=CategoricalActorDistribution(
            input_dim=256, 
    ...
    def forward(self, state: torch.Tensor) -> torch.Tensor:
        z=self.head.sample(self.net(state), True)[0]
        return torch.tanh(z)
...
class BC:
    ...
    def train(self, batch: TensorBatch) -> Dict[str, float]:
        ...
        # Compute actor loss
        pi = self.actor.net(state)
        actor_loss = - self.actor.head.evaluate(pi, action.detach(), is_onehot_action=True)[0]
...
def train(config: TrainConfig):
    exp_name = "_".join([config.env, "seed"+str(config.seed)]) 
    logger = CompositeLogger(log_path=f"./log/bc10/onestep/{config.job_name}", name=exp_name, loggers_config={
        "TensorboardLogger": {"activate": True},
        "WandbLogger": {"activate": False}
    })
    
    if config.env == "2048-v0":
        env, dataset = get_stochastic_dataset(config.env)
    else:
        env, dataset = get_d4rl_dataset(config.env, normalize_obs=False, normalize_reward=False, discard_last=False)
    ...
    for t in range(int(config.max_timesteps)):
        ...
        if (t+1) % config.log_freq ==0:
            logger.log_scalars("main_loop", log_dict, step=trainer.total_it)
        # Evaluate episode
