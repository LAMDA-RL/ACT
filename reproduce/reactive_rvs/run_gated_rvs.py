...
from acsl.module.net.reactive import GatedMLP
...
logger = CompositeLogger(log_path=f"./log/reactive_rvs/gated/onestep/{args.name}", name=exp_name, loggers_config={
    "FileLogger": {"activate": not args.debug},
    "TensorboardLogger": {"activate": not args.debug},
    "WandbLogger": {"activate": False, "config": args, "settings": wandb.Settings(_disable_stats=True), **args.wandb}
})
setup(args, logger)
product_root = f"./out/act/reactive_rvs/gated/{args.name}/{args.task}/seed{args.seed}/"
...
actor = GatedMLP(
    state_dim=obs_shape,
    state_hidden_dims=args.hidden_dims,
    adv_hidden_dims=args.hidden_dims,
    action_hidden_dims=args.hidden_dims,
    action_dim=action_shape,
    gate_dim=args.hidden_dims[-1],
